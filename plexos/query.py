...
class PlexosQuery:
    ...
    def _scan_data(self) -> None:
        ...
        self.t_period_0 = pl.scan_parquet(self.data_path / "t_period_0.parquet").with_columns(pl.col("datetime").str.to_datetime())
    ...
    def _join_all(self) -> pl.LazyFrame:
        ...
        return (
            self.t_membership
            .join(self.t_collection, on=["collection_id", "parent_class_id", "child_class_id"]).select(pl.exclude(["complement_name", "lang_id"]))
            .join(self.t_parent_object, left_on= "parent_object_id", right_on= "object_id", suffix= "_parent").select(pl.exclude(["class_id","category_id","index","show", "GUID"]))
            .join(self.t_child_object, left_on= "child_object_id", right_on= "object_id", suffix= "_child").select(pl.exclude(["class_id","index","show", "GUID"]))
            .join(self.t_property, on="collection_id", suffix="_property").select(pl.exclude(["enum_id","summary_name","summary_unit_id","is_multi_band","is_period","is_summary", "lang_id","unit_id"]))
            .join(self.t_key, on= ["membership_id", "property_id"]).select(pl.exclude(["model_id","period_type_id","band_id","sample_id","timeslice_id"]))
            .join(self.t_data_0, on="key_id")
            .join(self.t_phase_3, on="period_id")
            .join(self.t_period_0.select(["interval_id", "day_id", "period_of_day", "datetime"]), on="interval_id")
            .join(self.t_category.select(["category_id", "name"]), on="category_id", suffix="_category")
            .rename(
    ...
    def query_lazy_data(self, phase_id: int, collection_id: int, property_id: int) -> pl.LazyFrame:
        return (
            self.data
            .filter(
    ...
    def query_data(self, phase_id: int, collection_id: int, property_id: int) -> pl.DataFrame:
        return (
            self.data
            .filter(
                (pl.col("phase_id") == phase_id) &
